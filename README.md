...
`agent-image` 是 `agent-control` 调度用户 Runtime 时使用的 OpenCode 镜像构建仓库。它不负责 Runtime 调度、用户鉴权、Redis 租约、Kubernetes 资源管理或 Agent API 代理。
...
`agent-control` 负责控制面，`agent-image` 只负责运行面镜像。
...
| `agent-image` | OpenCode Runtime 镜像、默认 `/app` 项目骨架、启动命令和基础工具 | 调度、鉴权、Redis、Kubernetes 控制面、代理逻辑 |
...
`agent-image` 不处理 `scene` 字段；`scene` 的校验、转换和移除由 `agent-control` 完成。
...
`agent-image` 镜像内置的 `/app`、`AGENTS.md` 和 `.opencode/` 只作为默认示例。真实部署时，`agent-control` 通过 Kubernetes volume / volumeMount 注入用户工作目录与预设 scene 配置。
